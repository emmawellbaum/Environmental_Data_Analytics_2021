...
> Answer: 
H0 = mu >= 50 ppm The null hypothesis is that the average of our samples is greater than or equal to 50ppm.
Ha = mu < 50 ppm The alternative hypothesis is that the average of our samples is less than 50 ppm.
...
EPAair.subsample <- sample_n(EPAair, 5000) # Shapiro-Wilk test can only run on 5000 or fewer records
...
O3.onesample # p-value is << 0.05,reject null hypothesis.
...
library(corrplot)
EPA_Corr <- cor(EPAair %>% select(Mean.Latitude:PM2.5) %>% na.omit())
...
Ozone.regression <- lm(data=EPAair, PM2.5~Ozone + Year + Month + Mean.Longitude + Mean.Latitude)
summary(Ozone.regression)
step(Ozone.regression)
...
the_best_model <- lm(formula = PM2.5~Ozone + Year + Month + Mean.Longitude + Mean.Latitude, data = EPAair)

summary(the_best_model)
```

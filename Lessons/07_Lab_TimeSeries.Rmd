...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% # or NA instead of Null
...
ggplot(Temp_Components) +
  ...
  geom_line(aes(y = trend, x = Date), color = "Red", size = 1)
...
# Run SMK tests
rain_data_trend1 <- SeasonalMannKendall(ts_Rain)
rain_data_trend2 <- smk.test(ts_Rain)

temp_data_trend1 <- SeasonalMannKendall(ts_Rain)
temp_data_trend2 <- smk.test(ts_Rain)

# Inspect results: Rain
summary(rain_data_trend1)
summary(rain_data_trend2)

# Inspect results: Temp
summary(temp_data_trend1)
summary(temp_data_trend2)
...
ts_Rain_noseason <- ts(data = Rainfall_Components$Observed - Rainfall_Components$seasonal,
                       start = c(f_year,f_month),
                       frequency = 12)
...
ts_Temp_noseason <- ts(data = Temp_Components$Observed - Temp_Components$seasonal,
                       start = c(1901,1),
                       frequency = 12)

mk_Temp <- trend::mk.test(ts_Temp_noseason)
mk_Temp

# Plots
Temp_Components <- mutate(Temp_Components, Temp_noseason = Observed - seasonal)
Rainfall_Components <- mutate(Rainfall_Components, Rain_noseason = Observed - seasonal)

# Plotting resulting series: Temperature
ggplot(Temp_Components) +
  geom_line(aes(y = Observed, x = Date), size = 0.25, alpha = 0.3) +
  geom_line(aes(y = Temp_noseason, x = Date), color = "red") + # why not ts_Temp_noseason?
  geom_smooth(aes(y = Temp_noseason, x = Date)) +
  ylab("Temperature")

# Plotting resulting series: Rainfall
ggplot(Rainfall_Components) +
  geom_line(aes(y = Observed, x = Date), size = 0.25, alpha = 0.3) +
  geom_line(aes(y = ts_Rain_noseason, x = Date), color = "green") +
  geom_smooth(aes(y = Rain_noseason, x = Date)) +
  ylab("Rainfall")

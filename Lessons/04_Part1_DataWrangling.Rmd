...
`>`
`>=`
`&` # "and" symbol for comparing multiple commands and both are true
`|` # "or" symbol for multiple commands and only one can be true
...
# matrix filtering                
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] # filters matrix for rows where the depth column == 0

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) # same as above, but faster
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25) # depth < 0.25, same result
...
# filter for "Paul Lake" or "Peter Lake"
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")
# filter lakename to exclude lake that are not "Paul Lake" or "Peter Lake"
...
# examine actual data to find the appropriate daynum range for June thru October
# An "&" and "," both represent a combination of both
...
# The default is ascending order.Use "desc(column)" for descending order. 
...
```{r}
# Selects all columns between and including sampledate and temperature_C
...
```{r}
# add a month column to the dataset (containing the month information from sample date)
...
# first and last is a much better approach to calculating intervals that works with any size dataset

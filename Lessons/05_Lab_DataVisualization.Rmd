...
The expression function can be used when you want to add mathematical symbols and subscripts mixed with regular letter to labels. Subscript is represented using [x]. 
...
ex.1 <- 
  ...
  geom_point(size=0.5, shape=17, alpha=0.5) +
  facet_wrap(vars(lakename), nrow=2) +
  labs(title="Plot 1", x = "Day of the Year", y = "Temperature (C)", color = "Year") +
...
ex.3 <-
  ggplot(EPAair, aes(x=Ozone, y=PM2.5, color=Mean.Latitude)) +
  geom_point(size=0.5, alpha=0.5)+
...
# Column plot
ggplot(EPAair, aes(y=PM2.5, x=as.factor(Year))) +
  geom_col()
...
ggplot(EPAair, aes(x=as.factor(Year), y=PM2.5)) +
  geom_dotxplot(aes(color=as.factor(Year),fill=as.factor(Year)),
                binaxis = 'y',
                binwidth = 1,
                stackdir = 'center',
                dotsize = 0.2)
...
#Add a new attribute of sorted Endpoint levels
Neonics <- Neonics %>% 
  mutate(Endpoint_1 = reorder(Neonics$Endpoint,Neonics$Endpoint,FUN='length'))

# Change the factor order

ggplot(Neonics) +
  geom_bar(aes(x = Endpoint_1), fill="orange", color="dark orange") +
  theme_minimal() +
  labs(x='Endpoint') +
  theme(axis.text.x = element_text(angle=45))+
  scale_x_discrete(limits = rev(levels(Neonics$Endpoint_1)))

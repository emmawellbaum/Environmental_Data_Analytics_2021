  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(inputId = "y",
                  label = "Nutrient",
                  choices = c("Total Nitrogen (tn_ug)"="tn_ug",
                              "Total Phosphorous (tp_ug)"="tp_ug",
                              "Ammonia/Ammonium (nh34)"="nh34",
                              "Nitrogen Dioxide/Nitrate (no23)"="no23",
                              "Phosphate (po4)"="po4"), 
                  selected = "Total Nitrogen (tn_ug)"),
    ...
    # Create a ggplot object for the type of plot you have defined in the UI  
       output$scatterplot <- renderPlot({
        ggplot(filtered_nutrient_data(),#dataset
          ...
          labs(x = "Date", y = expression(Concentration ~ (mu*g/L)), fill = "Depth ID", shape = "Lake") +

...
matrix1[4, ] # row 4
matrix1[ , 3] # column 3
matrix1[c(12, 14)] # item in row 12, column 14
matrix1[c(12:14)] # slice of values in r/c 12-14
matrix1[2:4, 1:3] # slice in row:column format
...
# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1 # semicolon allows you to run two commands on one line
list1[[2]]
...
dataframe1[1:2,] # 
dataframe1[c("ID","Passed")] # specify column names
dataframe1$ID
...
> Answer: Different column names pop up.
...
```{r}
low_temp <- c(25,41,32,32,28,36,26,16,26,20)
high_temp <- c(53,55,54,49,53,56,53,42,37,42)
```
...
low_temp_C <- F_to_C(F = low_temp)
low_temp_C
high_temp_C <- F_to_C(F = high_temp)
high_temp_C
```
...
```{r}
temp_df <- data.frame(low_temp, high_temp, low_temp_C, high_temp_C)
names(temp_df) <- c("low_F","high_F","low_C","high_C"); View(temp_df)
```

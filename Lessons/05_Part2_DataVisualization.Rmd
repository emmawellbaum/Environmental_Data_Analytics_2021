...
#base_size (font) is 11 in default ggplot
...
theme_set(mytheme) #set theme beforehand for all plots
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + # add horizontal line, y intercept = 50
  geom_hline(yintercept = 100, lty = 2) + 
  geom_point(alpha = 0.5, size = 1.5) + # add points representing the data
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") + # change axis scale, breaks, and labels
  ylab(expression("O"[3]* " AQI Value")) + # change y axis label
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) # change x axis text
print(O3plot7)  

...
# Example with aesthetics in ggplot line (applied to all plots in ggplot)
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point()

# Example with aesthetics in geom line (useful when adding 2+ plots)
...
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue")) -- color inside aes adds a plot layer
  geom_point(aes(x = Date, y = Ozone), color = "blue") # color outside aes changes plot color
print(O3plot2)
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #separate plots for each site, organized into 3 rows
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) # site name by year (column representing each year and rows representing the three sites)
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() # because temperature decreases with depth
print(tempvsdepth)
...
NvsP <-
  ...
  geom_abline(aes(slope = 16, intercept = 0)) # line when slope and intercept are known
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Becuase it would have left the outline of each boxplot black and changed the fill color of based on nutrient type. This would have been difficult to see.
print(Nutrientplot3)
...
print(Nutrientplot6) #inefficient method; use gathered dataset
...
Nutrientplot6 <-
  ...
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) # alternative to frequency plots
print(Nutrientplot6)

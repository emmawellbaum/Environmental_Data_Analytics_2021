...
R Markdown is useful because it allows users to:

-   Create an *easy to read* document directly from the markdown file
-   Create files in *various formats* for easy sharing
-   Utilize **version control**
-   Easily combine text/code/plots in one document
...
Dataset                       | Description
----------------------------- | ------------------------------------------------
ECOTOX Neonicotinoids Insects | Neonicotinoid effects on insects
EPA Air O~3~ NC2018 & NC2019  | Daily ozone levels at all NC sites (2018-19)
EPA Air PM25                  | Daily PM2.5 levels at all NC sites (2018-19)
EPA Air 0~3~ Garinger NC      | Daily O~3~ levels at Garinger, NC site (2010-19)
NEON NIWO Litter              | Mass & trap sampling datasets
NTL-LTER chemical & physical  | lake depth, temperature, oxygen, and light data
NTL-LTER nutrient             | lake nutrient data (phosphorous, nitrogen, etc.)
NWIS Site Info NE             | USGS gage site locations in NE
NWIS Site FLow Data NE        | USGS gage site flow data for NE
...
# Customize the chunk options so that the code is run, but not displayed
...
```{r process.data}
NTL.nutrient.processed <- NTL.nutrient.data %>% 
...
NTL.Nitrogen <- NTL.nutrient.processed %>%
  group_by(lakename) %>%
...
NTL.Phosphorous <- NTL.nutrient.processed %>%
  group_by(lakename) %>%
...
knitr::kable(NTL.Nitrogen, col.names = c('Lake Name', 'Mean TN', 'Min. TN', 'Max. TN', 'SD'), caption="Summary of Total Nitrogen (TN) by Lake (in ug)")

knitr::kable(NTL.Phosphorous, col.names = c('Lake Name', 'Mean TP', 'Min. TP', 'Max. TP', 'SD'), caption="Summary of Total Phosphorous (TP) by Lake (in ug)")
```
...
```{r plot.TN, echo=FALSE, fig.align='left', fig.cap="Total Nitrogen Over Time"}
ggplot(NTL.nutrient.processed, aes(x=sampledate, y=tn_ug, color=lakename)) +
  geom_line() +
...
```{r plot.TP, echo=FALSE, fig.align='left', fig.cap="Total Phosphorous Over Time"}
ggplot(NTL.nutrient.processed, aes(x=sampledate, y=tp_ug, color=lakename)) +
  geom_line() +
  scale_color_manual(values = c('#332288','#117733', '#44AA99', '#88CCEE', '#DDCC77', '#CC6677', '#AA4499', '#882255')) +
  labs(x='Date', y ='Total Phosphorous (ug)', color='Lake Name')
```
...
Total Nitrogen and phosphorous levels vary across the eight lakes included in the North Temperate Lakes Long Term Ecological Research studies. While the overall trend for some lakes, such as West Long Lake, appears to be fairly horizontal for both nutrients over time, other lakes, such as East Long Lake, have experienced noticeable increases in nutrient levels over time. When looking at the eight lakes together, there are also apparent spikes in nutrient levels during certain years. For example, there was a spike in both total nitrogen and phosphorous levels affecting several lakes during the summer of 1996.
***
To analyze this dataset further, we could perform trend analyses for each of the NTL-LTER lakes to determine whether there have been any significant changes in lake nutrient levels over time. In doing so, it will be necessary to evaluate whether lake nutrient levels are seasonal. This trend analysis would also allow us to compare the lakes to each other and ultimately evaluate the lake health throughout the NTL-LTER.
